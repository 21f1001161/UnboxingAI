...
function PlaylistItem({ story, index, completed, open, markCompleted, togglePlaylist }) {
  return <article className={`playlist-item${completed ? ' done' : ''}`}>
    ...
    <div className="playlist-item-actions">
      <button onClick={() => markCompleted(story.id)} className={completed ? 'completed' : ''}>{completed ? 'Completed' : 'Mark as complete'}</button>
      <button onClick={() => togglePlaylist(story.id)} className="remove-action">Remove</button>
    </div>
  </article>;
}
...
function QueueItem({ story, index, inPlaylist, read, completed, open, togglePlaylist, markCompleted }) {
  return <article className={`queue-card${completed ? ' done' : ''}`}>
    ...
    <footer>
      ...
      <button onClick={() => markCompleted(story.id)} className={completed ? 'completed-state' : ''}>{completed ? 'Completed' : 'Complete'}</button>
      <button onClick={() => togglePlaylist(story.id)} className={inPlaylist ? 'in-playlist' : 'add-playlist'}>{inPlaylist ? 'In playlist' : '+ Playlist'}</button>
    </footer>
  </article>;
}

export function LearningDashboardV2({ saved, playlist, togglePlaylist, open, learning, setLearning, markCompleted, clearCompletedPlaylist }) {
  ...
  const orderedEntries = useMemo(() => [...entries].sort((a, b) => {
    const aState = completed.includes(a.id) ? 2 : read.includes(a.id) ? 1 : 0;
    const bState = completed.includes(b.id) ? 2 : read.includes(b.id) ? 1 : 0;
    return aState - bState;
  }), [completed, entries, read]);
